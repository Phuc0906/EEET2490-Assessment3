...
void uart_init()
{
	...
	r = GPFSEL3;
	// clear bit at GPIO32 and 33 first
	r &= ~((7 << 6) | (7 << 9));
	r |= (0b111 << 6) | (0b111 << 9); // configure to alt 3
	GPFSEL3 = r;
        ...
        GPPUDCLK1 = (1 << 32)|(1 << 33); //enable clock for GPIO 14, 15
        r = 150; while(r--) { asm volatile("nop"); } //waiting 150 cycles
        GPPUDCLK1 = 0;        // flush GPIO setup
    #else //RPB4
        r = GPIO_PUP_PDN_CNTRL_REG2;
        r &= ~((3 << 0) | (3 << 2)); // Clear Resistor Select: GPIO_PUP_PDN_C NTRL15, GPIO_PUP_PDN_C NTRL14
...
void uart_hex_custom(unsigned int num) {
	int i = 0;
	for (int pos = 28; pos >= 0; pos = pos - 4) {
		// Get highest 4-bit nibble
		char digit = (num >> pos) & 0xF;
